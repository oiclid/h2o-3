...
public class GLRMTest extends TestUtil {
  ...
  @Test public void testWideDataSetGLRM() throws InterruptedException, ExecutionException {
    Scope.enter();
    ...
    try {
      train = parse_test_file(Key.make("smalldata/prostate/prostate_cat.csv"), "smalldata/prostate/prostate_cat.csv");
      Scope.track(train);
      DKV.put(train);
      ...
      glrmParms.setWideDataset(true);  // force to treat dataset as wide even though it is not.
      modelW = glrmParms.trainModel().get();
      scoreW = modelW.score(train);
      Scope.track(scoreW);
      Scope.track_generic(modelW);

      // compare eigenvectors and eigenvalues generated by original GLRM and wide dataset GLRM.
